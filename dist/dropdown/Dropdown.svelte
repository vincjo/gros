<script lang="ts">import { clickOutside } from '../action';
import { fade } from 'svelte/transition';
import { createPopperActions } from '../tooltip';
import { onMount } from 'svelte';
let { position = 'bottom', preventClosing = false, fixedWidth = false, isBlock = false, active = false, disabled = false, onclick = () => { return; }, content, children, } = $props();
let minWidth = $state('auto');
let element;
let dropdownElement = $state(undefined);
onMount(() => {
