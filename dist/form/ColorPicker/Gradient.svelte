<script lang="ts">import { preventDefault, stopPropagation } from '../../event-modifier';
let { handler } = $props();
let element;
...
const mousemove = (e) => {
    if (isMouseDown) {
        update({
            offsetX: Math.max(0, Math.min(element.getBoundingClientRect().width, e.clientX - element.getBoundingClientRect().left)),
            offsetY: Math.max(0, Math.min(element.getBoundingClientRect().height, e.clientY - element.getBoundingClientRect().top))
        });
    }
};
const mouseup = () => {
